...
function setPillHotRect(rect: PillHotRect | null | "pass-through"): void {
  ...
  const hot = rect === "pass-through" ? null : rect;
  pillHotRect = hot;
...
app.whenReady().then(async () => {
  ...
  ipcMain.on("pill:set-hot-rect", (_event, rect: unknown, options: unknown) => {
    if (rect === null) {
      const passThrough =
        typeof options === "object" &&
        options !== null &&
        (options as Record<string, unknown>).passThrough === true;
      setPillHotRect(passThrough ? "pass-through" : null);
      return;
    }
