...
/**
 * How many reads one turn gets, counted across every scope.
 *
 * The identical-call breaker above keys on the arguments, which reads can
 * vary for free: `selection`, `document`, `surroundings`, `near-cursor` and
 * `clipboard` are five separate budgets to the fingerprint, so a model that
 * cycles scopes never trips it. One observed turn read seven times without
 * repeating itself once. Four views of one document is already more than any
 * real task needs, and a fifth is the loop signature, not diligence.
 */
export const REMIX_TURN_READ_LIMIT = 4;
...
export interface RemixSession {
  ...
  /** Per-tool tallies this turn, keyed by tool name alone. */
...
export async function runRemixTool(
  ...
  const named = (session.toolCalls.get(name) ?? 0) + 1;
  if (name === "read_writing_context" && named > REMIX_TURN_READ_LIMIT) {
    return fail("read-budget-spent", {
      retryable: false,
      detail: `this turn has already read the writing context ${named - 1} times, across every scope`,
      next: "You have looked at this document from every angle it has. Work with what you already read, or tell the user what you could not find.",
    });
  }
  session.toolCalls.set(name, named);
