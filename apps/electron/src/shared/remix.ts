...
/**
 * The tri-state reading for a host that only sent us a string.
 *
 * Legacy hosts could not tell "nothing highlighted" from "read failed", and
 * treated both as compose-at-cursor — so that is what the absent case maps to.
 */
export function targetFromSelection(
  selection: string | null,
): RemixSelectionState {
  return selection ? { status: "selected", text: selection } : { status: "empty" };
}
