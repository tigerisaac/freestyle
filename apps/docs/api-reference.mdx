...
| `GET` | `/stream` | WebSocket endpoint for real-time streaming transcription. Handles binary audio frames and JSON control messages (`context`, `start`, `commit`, `cancel`). |
...
| Method | Path | Description |
| --- | --- | --- |
| `ALL` | `/mcp` | MCP Streamable HTTP transport endpoint. See the [MCP integration guide](/mcp) for details. |
