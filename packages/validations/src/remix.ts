...
export const REMIX_CLIENT_TOOLS = {
  ...
  remember_for_this_piece: {
    description:
      "Record something about this piece of work that later turns will need and would otherwise have to rediscover by re-reading the whole document: what the piece is for (brief), the structure you agreed (outline), a fact you established or checked (fact), a source you used (source), or a question still open (open-question). Returns { ok }. For long-form work only — a few durable things, not a running log; a short edit needs none of this. Recording a brief or an outline replaces the previous one. Stored on the user's own machine and carried into later turns of this thread automatically, so you never need to read it back.",
    inputSchema: z.object({
      kind: z
        .enum(["brief", "outline", "source", "fact", "open-question"])
        .describe(
          "Which kind of thing this is. 'brief' and 'outline' are singular per piece and replace what was there.",
        ),
      content: z
        .string()
        .min(1)
        .max(2_000)
        .describe(
          "The thing itself, written so a later turn with none of this conversation can act on it.",
        ),
    }),
  },
} as const;
